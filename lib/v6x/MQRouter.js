...
class MQRouter {
  ...
  consumeMessages({
    ...
    return _asyncToGenerator(function* () {
      try {
        const unserializedMessage = yield _this9.serializer.unserialize(message);
  ...
  buildRequest({ message, replyId = '', to = '' }) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const m = _this12.mqStructure({
        id: _this12.getMessageId(),
        replyTo: replyId,
      ...
      const serializedMessage = yield _this12.serializer.serialize(m);
      return {
        message: serializedMessage,
        id: m.id
      };
