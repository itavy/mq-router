...
class MQRouter {
   ...
   * @param {Boolean} [duplicate=true] if true, allow multiple bindings per queue
  ...
  subscribe({
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        ...
        consumerTag = _queuesRoutingTable$r.consumerTag;
  ...
  unsubscribe(mqProps) {
    ...
    let index = null;
    try {
      var _queuesRoutingTable$g = this.queuesRoutingTable.getHandlerRefsByProperties(mqProps);

      consumerTag = _queuesRoutingTable$g.consumerTag;
      index = _queuesRoutingTable$g.index;

      if (this.queuesRoutingTable.unregister({ index })) {
        this.connector.unsubscribe({ consumerTag });
        return true;
      }
    } catch (cause) {
  ...
  checkIfIsSelfSubscribedForResponses() {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this6.identification.subscribing = true;
      try {
        var _ref3 = yield _this6.subscribe({
          // this call will be covered in e2e tests
          handler: /* istanbul ignore next */function handler(...args) {
            return _this6.ownHandler(...args);
          },
          queue: _this6.identification.listen.queue,
          topic: _this6.identification.listen.topic,
          exchange: _this6.identification.listen.exchange,
          options: {
            prefetch: 0,
            exclusive: true
          }
        ...
        _this6.identification.subscribed = true;
  ...
  consumeMessages({
    ...
    return _asyncToGenerator(function* () {
      try {
        const unserializedMessage = yield _this8.mqMessage.from(message);
  ...
  close() {
    ...
    return _asyncToGenerator(function* () {
      yield _this13.connector.close();
      return _this13.requestsRoutingTable.close();
    })();
  }
}
