...
const MS_FACTOR = _require.MS_FACTOR,
      NS_FACTOR = _require.NS_FACTOR,
      IError = _require.IError;

var _require2 = require('./MQQueuesRoutingTable');

const MQRequestsRoutingTable = _require2.MQRequestsRoutingTable;

var _require3 = require('./MQQueuesRoutingTable');

const MQQueuesRoutingTable = _require3.MQQueuesRoutingTable;

var _require4 = require('@itavy/mq-connector');

const getConnector = _require4.getConnector,
...
class MQRouter {
  ...
  constructor({
    ...
    this.requestsRoutingTable = Reflect.construct(MQRequestsRoutingTable, [{ name }]);
    this.requestsRoutingTable.setMessagesTimeoutListener({
      emitter: this.mqrEvents
    });

    this.queuesRoutingTable = Reflect.construct(MQQueuesRoutingTable, [{ name }]);
  ...
  sendMQMsg({
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        ...
        _this3.requestsRoutingTable.unregister({
          id,
          error
        });
      }
