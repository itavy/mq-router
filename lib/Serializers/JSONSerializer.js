  ...
  serialize(request) {
    return new Promise((resolve) => {
      const sMessage = JSON.stringify({
        ...
        message:   this.utils.stringFromUint8Array(request.message),
      });
  ...
  unserialize(request) {
    return new Promise((resolve) => {
      ...
      return resolve({
        ...
        message:   this.utils.stringToUint8Array(request.uMessage),
      });
    })
    .catch(err => Promise.reject(this.utils.createMQError({
      name:  '',
...
/**
 * [getSerializer description]
 * @param  {Object} di dependencies required for MQ JSON Serializer
 * @return {MQJSONSerializer}    instance of MQJSONSerializer
 */
const getSerializer = di => new MQJSONSerializer(di.utils.validateConstructorDependencies(di));
