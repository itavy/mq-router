...
class MQJSONSerializer {
  ...
  serialize(request) {
    return new Promise((resolve) => {
      const sMessage = this.utils.serializeJSON({
        msgId:     request.msgId,
  ...
  unserialize(request) {
    return new Promise((resolve) => {
      const uMessage = this.utils.unserializeJSON(this.utils.stringFromUint8Array(request));
      return resolve({
 ...
 */
const getSerializer = di => Reflect.construct(
  MQJSONSerializer,
  [
