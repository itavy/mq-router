...
class RabbitMQ {
  ...
  subscribe(request) {
    return this.setupSubscribe(request)
      ...
      .then(queueSetup => (queueSetup.channel.consume(queueSetup.queue, message => (
        ...
        .then((/* respSubscribe */) => {
          this.connection.subscribedChannels.set(request.rkey, queueSetup.channel);
          return Promise.resolve();
        })
      ))
  ...
  setupChannel(request) {
    return this.connection.instance.createConfirmChannel()
      ...
      .then(ch => (ch.assertExchange(this.connection.exchange.name, 'topic', this.connection.exchange.options)
        .then(() => Promise.resolve(ch))
      ))
  ...
  setupSubscribe(request) {
    return this.setupChannel(request)
      ...
      .then(ch => (
        ch.channel.bindQueue(ch.queue.queue, this.connection.exchange.name, request.rkey)
          .then(() => Promise.resolve(ch))
      ))
      // resolve back
